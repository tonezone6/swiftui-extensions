...
extension String {
  private static var lipsum_words_200: String {
  ...
  public static func loremIpsum(words count: Int = 8) -> String? {
    ...
    let components = String.lipsum_words_200.components(separatedBy: charset)
    let words = components.filter { !$0.isEmpty }
    let array = words.prefix(count)
    return array.joined(separator: " ").sentenceCapitalized
  }
